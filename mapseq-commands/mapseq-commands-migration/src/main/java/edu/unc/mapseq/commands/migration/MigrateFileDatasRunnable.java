...
public class MigrateFileDatasRunnable implements Runnable {
    ...
    private final Logger logger = LoggerFactory.getLogger(MigrateFileDatasRunnable.class);
                    ...
                    if (samples != null && !samples.isEmpty()) {
                        ...
                        for (Sample sample : samples) {
                            ...
                            for (Workflow workflow : workflows) {
                                ...
                                File sampleDir = new File(String.format("%s/%s/%s/L%03d_%s", basePath,
                                        flowcell.getName(), workflow.getName(), sample.getLaneIndex(),
                                        sample.getBarcode()));
                            ...
                            if (!dryRun) {
                                ...
                                sample.setOutputDirectory(String.format("%s/%s/L%03d_%s", mpsOutputDir,
                                        flowcell.getName(), sample.getLaneIndex(), sample.getBarcode()));
